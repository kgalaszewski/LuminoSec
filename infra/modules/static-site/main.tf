locals {
  zone_name = coalesce(var.zone_name, var.domain_name)
}
...
# --- S3 bucket holding the site content (private, only CloudFront can read it) ---

resource "aws_s3_bucket" "site" {
  bucket = "luminosec-site-${var.environment}-${data.aws_caller_identity.current.account_id}"
}
...
resource "aws_cloudfront_origin_access_control" "site" {
  name                              = "luminosec-${var.environment}-site-oac"
